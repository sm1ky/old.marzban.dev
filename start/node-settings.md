...
После получения необходимых сертификатов от панелей и размещения их в указанных местах, запустите Марзбан Нод.

Порты подключения нода к панелям и порты, доступные для входящих соединений, будут следующими:
...
**Совет первый**
Если для лучшего управления нодами вы хотите назначить каждому ноду отдельный входящий канал, вам нужно добавить новый входящий канал с уникальным тегом и портом в настройках ядра (Core Settings).

**Совет второй**
Если вы хотите использовать Warp на сервере нода и настроили файл `docker-compose.yml` с использованием второго способа, вам нужно активировать Warp с ядром Xray. Если вы используете ядро Wireguard, Warp на сервере нода не будет работать.

**Совет третий**
Если вы планируете использовать конфигурации с TLS, вам нужно получить сертификат для вашего домена на сервере нода, затем перенести его на основной сервер и указать путь к файлам сертификата в настройках входящего канала. Кроме того, вместо нескольких сертификатов для разных субдоменов, вы можете получить один Wildcard сертификат для основного домена, чтобы использовать его для всех субдоменов.
...
**Совет пятый**
Если вы внесли изменения в файл `docker-compose.yml`, перезапустите Марзбан Нод с помощью следующей команды:
