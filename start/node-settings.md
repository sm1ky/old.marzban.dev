...
В связи с изменением принципа авторизации между Центральной панелью и узлом, и переходом на mTLS, алгоритм подключения узла теперь немного отличается от привычного.
### Получение ключа Центральной панели
Открываем настройки узлов и переходим в добавление нового узла.

![](/static/node3.jpg)

Скачиваем сертификат в файловую систему, 
![](/static/node1.jpg)

или нажимаем значок просмотреть и записываем полученное значение.
![](/static/node2.jpg)
...
Теперь нам необходимо установить ключ центальной панели на узел, для этого мы можем скопировать ранее полученный ключ по пути `/var/lib/marzban-node/ssl_client_cert.pem`
или Вы скопировали значение ключа ранее просто вставив его в нужный файл.

```bash
nano /var/lib/marzban-node/ssl_client_cert.pem
```

После этого Вы сможете запустить узел.
