...
{% hint style="warning" %}
Данный функционал пока в тестовом режиме.
Панель и узел должны быть взаимно обновлены для поддержки этого.
{% endhint %}
## Отличие REST от RPC
В последнем обновлении узлов (0.3.0) и в последней версии панели, была  добавлена поддержка REST, который, в дальнейшем, должен прийти на замену  RPC.

REST и RPC — это два различных подхода к организации взаимодействия между различными частями распределенных систем, в нашем случае между панелью и узлом. 

Для простоты понимания, рассмотрим  основные различия.

**REST**:
REST основан на использовании стандартных HTTP-методов (GET, POST, PUT, DELETE и т.д.) для выполнения операций с ресурсами.
Поскольку REST использует стандартные HTTP-методы, его легко понимать и использовать.
Благодаря безсостояниевому взаимодействию и кешированию, REST-сервисы могут быть хорошо масштабированы.

**RPC**:
RPC сосредоточен на вызове удаленных процедур, где взаимодействие организовано в виде вызова функций или методов с передачей параметров.
RPC позволяет напрямую вызывать удаленные процедуры, что может быть более интуитивно понятно разработчикам.
Но, клиент и сервер в RPC теснее связаны, что может затруднить их независимое развитие.
использование:

Обьявляем в `docker-compose.yml` файле нашего проекта, о использовании REST, открыв его через `nano` 
```bash
nano docker-compose.yml
```
